...
function applyFibonacciToSignal(modifier) {
  ...
  // Añadir razón Fibonacci al logicText con zona y nivel
  if (logicText && modifier.reason && !logicText.textContent.includes('📐')) {
    const fibZone = fibState.currentZone ? fibState.currentZone.label : '';
    const fibDir  = fibState.direction === 'bullish' ? '↗' : '↘';
    logicText.textContent += ` | 📐 Fib ${fibDir}${fibZone ? ' '+fibZone : ''}: ${modifier.reason}`;
  }
}
...
var _reversalWatchCooldownIdx = -1; // prevents re-setting on consecutive velas
// Estructura: {
...
function update() {
  ...
  // Ventana extendida: hasta 6 velas para detectar batallas y patrones de swing
  const lastTwo = effectiveHistory.slice(-6);
    ...
    if (latestIdx > watchIdx && latestIdx === watchIdx + 1) {
      ...
      if (newCandle === reversalWatch.confirmColor) {
        ...
        logicText    = `🎯 Reversión ${dir === 'bullish' ? 'alcista' : 'bajista'} CONFIRMADA. ` +
                       `Proyección ${reversalWatch.confirmConf}% cumplida — vela ${newCandle === 'G' ? 'verde' : 'roja'} F${newStr}. ` +
                       (proj && proj.patternName ? `Patrón activo: ${proj.patternName}.` : '');
        showReversal = true;
        reversalWatch = null;
        ...
        reversalWatch = null;
        // If cancelling candle is weak (Doji F1-F1.5), generate ESPERAR instead of directional signal
        if (newStr <= 1.5) {
          finalSignal = 'wait'; signalClass = 'signal-conflict'; signalText = '⚠️ ESPERAR';
          logicText = `↩️ Reversión ${dir === 'bullish' ? 'alcista' : 'bajista'} cancelada por vela débil F${newStr}. Sin dirección clara — esperar confirmación.`;
        } else {
      ...
      reversalWatch = null;
      _reversalWatchCooldownIdx = latestIdx - 1; // allow new watch after gap
    }
  }
    ...
    if (extremeCondition.type === 'extreme_bullish') {
      ...
      if (currentCandle.current === 'R' && currentCandle.strength >= 2 && !hasConflict) {
        if (isFirstRedAfterBullRun || isWeakFirstRed) {
          ...
          logicText = `🚨 SOBRECOMPRA EXTREMA MA10 ${trendMA !== null ? trendMA.toFixed(0) : "?"}% · Reversión bajista F${currentCandle.strength}` +
                      (proj && proj.patternName ? ` · Patrón: ${proj.patternName} ${Math.round((proj.confidence||0)*100)}%` : '') +
                      (prevGreensBeforeRed > 0 ? ` · ${prevGreensBeforeRed} verde(s) previas agotadas` : '');
        }
      } else if (hasConflict) {
        finalSignal = 'wait'; signalClass = 'signal-conflict'; signalText = '⚠️ ESPERAR';
      ...
      if (currentCandle.current === 'G' && currentCandle.strength >= 2 && !hasConflict) {
        if (isFirstGreenAfterBearRun || isWeakFirstGreen) {
          ...
          logicText = `🚨 SOBREVENTA EXTREMA MA10 ${trendMA !== null ? trendMA.toFixed(0) : '?'}% · Reversión alcista F${currentCandle.strength}` +
                      (proj && proj.patternName ? ` · Patrón: ${proj.patternName} ${Math.round((proj.confidence||0)*100)}%` : '');
        } else {
  ...
  if (finalSignal === 'wait' && !showExtreme) {
    ...
    if (cmdState.phase === 'distribution' && cmdState.probability > 0.6) {
      ...
      if (candleOk && ma10Ok) {
        ...
        logicText = `🏦 DISTRIBUCIÓN ${Math.round(cmdState.probability*100)}% · Vela roja F${currentCandle.strength} · MA10 ${trendMA !== null ? trendMA.toFixed(0) : '?'}%` +
          (proj && proj.patternName ? ` · Patrón: ${proj.patternName} ${Math.round((proj.confidence||0)*100)}%` : '') +
          ' · Smart money vendiendo';
      } else if (ma10Ok) {
      ...
      if (candleOk && ma10Ok) {
        ...
        logicText = `📥 ACUMULACIÓN ${Math.round(cmdState.probability*100)}% · Vela verde F${currentCandle.strength} · MA10 ${trendMA !== null ? trendMA.toFixed(0) : '?'}%` +
          (proj && proj.patternName ? ` · Patrón: ${proj.patternName} ${Math.round((proj.confidence||0)*100)}%` : '') +
          ' · Smart money comprando';
      } else if (ma10Ok) {
    ...
    // ═══════════════════════════════════════════════════════════════════════
      ...
      if (currentCandle.current === 'G') {
        ...
        //   redStrengths: redStrengths.reverse()
        // });
        ...
        if (isBearTrapPattern || isEarlyReversal) {
          ...
          logicText = `🪤 Bear Trap: ${prevReds} rojas previas [${redStrengths.reverse().join('→')}] ` +
                      ...
                      (proj && proj.patternName ? `Patrón: ${proj.patternName}. ` : '') +
                      `⚡ NO COMPRAR aún — esperar segunda vela verde F3+.`;
          ...
          logicText = `🔄 Reversión alcista — ${prevReds} rojas previas [${redStrengths.reverse().join('→')}] ` +
                      ...
                      (proj && proj.patternName ? `Patrón: ${proj.patternName} ${Math.round((proj.confidence||0)*100)}%. ` : '') +
                      `Vela verde F${currentCandle.strength} confirma cambio.`;
          ...
          logicText = `⚠️ Reversión alcista débil — solo ${prevReds} rojas previas ` +
                      `[${redStrengths.reverse().join('→')}]. ` +
                      (proj && proj.patternName ? `Patrón: ${proj.patternName}. ` : '') +
                      `Confirmar con siguiente vela.`;
          
        } else {
          // Verificar si hay zona de batalla activa (alternaciones fuertes)
          if (proj && proj.patternName === 'Zona de Batalla') {
        ...
        if (isBullTrapPattern || isEarlyReversal) {
          ...
          logicText = `🪤 Bull Trap: ${prevGreens} verdes previas [${greenStrengths.reverse().join('→')}] ` +
                      ...
                      (proj && proj.patternName ? `Patrón: ${proj.patternName}. ` : '') +
                      `⚡ NO VENDER aún — esperar segunda vela roja F3+.`;
          ...
          logicText = `⚠️ Reversión bajista débil — solo ${prevGreens} verdes previas. ` + (proj && proj.patternName ? `Patrón: ${proj.patternName}. ` : '') + `Confirmar.`;
    ...
    // PRIORIDAD 5: CRUCE VÁLIDO (solo si es fresco — detectado en esta misma vela)
      ...
      if (trendMA >= 85 && activeCross.crossType === 'golden') {
        ...
        // Minimum margin to override Death Cross with bullish projection: 12pp
        // Below this threshold the signal is noise against a structural bearish cross
        const _dcMinMargin = 12;
        const _dcRevAlcista = proj && proj.ifG.sig === 'COMPRA'
          && proj.ifG.conf > proj.ifR.conf
          && (Math.round(proj.ifG.conf*100) - Math.round(proj.ifR.conf*100)) >= _dcMinMargin;
        if (tc && tc.type === 'bear-trap' && tc.probability >= 0.55) {
          ...
          logicText = `⚠️ Death Cross en ${activeCross.pair} pero proyección verde supera roja por ${diff}pp (${_dcGConf}% vs ${_dcRConf}%). Vigilando próxima vela...`;
          if (crossIsFresh && (!reversalWatch || reversalWatch.candleIndex !== history.length - 1)) {
          ...
          // Death Cross + sobreventa = zona de peligro en ambas direcciones → ESPERAR
          const _dcSmallDiff = proj ? Math.abs(_dcGConf - _dcRConf) : 0;
          if (_dcSmallDiff < 12) {
            finalSignal = 'wait'; signalClass = 'signal-conflict'; signalText = '⚠️ ESPERAR — ZONA COMPLEJA';
            logicText = `⚠️ Death Cross ${activeCross.pair} + MA10 ${trendMA.toFixed(0)}% (sobreventa) + proyecciones muy parejas (${_dcGConf}% vs ${_dcRConf}%). Sin ventaja clara — esperar ruptura definitiva.`;
          } else {
        ...
        const _gcMinMargin = 12;
        const _gcRevBajista = proj && proj.ifR.sig === 'VENTA'
          && proj.ifR.conf > proj.ifG.conf
          && (_gcRConf - _gcGConf) >= _gcMinMargin;
        if (_gcRevBajista && !(tc && tc.type === 'bull-trap')) {
          ...
          if (proj && proj.ifR.sig === 'VENTA' && proj.ifR.conf > proj.ifG.conf) {
            ...
            logicText = `✅ Golden Cross ${activeCross.pair} — MA10: ${trendMA !== null ? trendMA.toFixed(0) : '?'}%` +
              (proj && proj.patternName ? ` · Patrón: ${proj.patternName} ${Math.round((proj.confidence||0)*100)}%` : '') +
              ` · Verde ${_gcConfG}% vs Roja ${_gcConfR}%`;
          }
        }
      }

    // PRIORIDAD 6: PATRÓN + CONTEXTO
    } else if (proj && proj.patternName === 'Zona de Batalla') {
      finalSignal = 'wait'; signalClass = 'signal-conflict'; signalText = '⚔️ ZONA DE BATALLA';
      logicText = `⚔️ ${proj.context} · Velas fuertes alternadas sin ganador. Verde→${proj.ifG.sig} ${Math.round(proj.ifG.conf*100)}% / Roja→${proj.ifR.sig} ${Math.round(proj.ifR.conf*100)}%. Esperar ruptura.`;
      ...
      if (recentTrend.isExtreme && trendMA === null) {
        ...
        if (proj.ifR.sig === 'VENTA' && proj.ifR.conf > proj.ifG.conf && (confPctR - confPctG) >= 12) {
          const diff = confPctR - confPctG;
          finalSignal = 'wait'; signalClass = 'signal-conflict'; signalText = '⚠️ POSIBLE REVERSIÓN BAJISTA';
          logicText = `⚠️ Tendencia alcista pero proyección roja supera verde por ${diff}pp (${confPctR}% vs ${confPctG}%). Vigilando próxima vela...`;
          const _p5WatchIdx = history.length - 1;
          if ((!reversalWatch || reversalWatch.candleIndex !== _p5WatchIdx) && _reversalWatchCooldownIdx < _p5WatchIdx - 1) {
            reversalWatch = {
              direction: 'bearish',
              confirmColor: 'R', cancelColor: 'G',
              confirmConf: confPctR, cancelConf: confPctG,
              candleIndex: _p5WatchIdx,
              detectedAt: Date.now()
            };
        ...
        const confPctR = Math.round(proj.ifR.conf * 100);
        const confPctG = Math.round(proj.ifG.conf * 100);
        if (proj.ifG.sig === 'COMPRA' && proj.ifG.conf > proj.ifR.conf && (confPctG - confPctR) >= 12) {
          const diff = confPctG - confPctR;
          finalSignal = 'wait'; signalClass = 'signal-conflict'; signalText = '⚠️ POSIBLE REVERSIÓN ALCISTA';
          logicText = `⚠️ Tendencia bajista pero proyección verde supera roja por ${diff}pp (${confPctG}% vs ${confPctR}%). Vigilando próxima vela...`;
          const _p5aWatchIdx = history.length - 1;
          if ((!reversalWatch || reversalWatch.candleIndex !== _p5aWatchIdx) && _reversalWatchCooldownIdx < _p5aWatchIdx - 1) {
            reversalWatch = {
              direction: 'bullish',
              confirmColor: 'G', cancelColor: 'R',
              confirmConf: confPctG, cancelConf: confPctR,
              candleIndex: _p5aWatchIdx,
              detectedAt: Date.now()
            };
  ...
  if (_sinDatos) {
    ...
    if (_agotamiento && _lastColor === 'R' && _projDominante !== 'bajista') {
      ...
      // FIX: con datos insuficientes nunca emitir señal direccional
      // El patrón puede apuntar al alza/baja pero sin MA10 es ruido, no señal
      signalText  = '⏳ ESPERAR · DATOS INSUFICIENTES';
      signalClass = 'signal-wait';
      var _orientacion = _projDominante === 'alcista'
        ? ' · Patrón apunta al alza ' + Math.round(_projGConf*100) + '% (orientativo)'
        : _projDominante === 'bajista'
        ? ' · Patrón apunta a la baja ' + Math.round(_projRConf*100) + '% (orientativo)'
        : '';
      logicText   = '⏳ Solo ' + history.length + ' de 10 velas mínimas.'
        + _orientacion
        + ' · Registra ' + _faltanVelas + ' velas más para activar el análisis completo.';
    }
...
// ── SYNC: única llave maestra cmd-bt-seq ──────────────────────
// _suppressSyncWrite evita loop: cuando recibimos de BT, no re-escribimos
var _suppressSyncWrite = false;

function syncToBacktesting() {
  if (_suppressSyncWrite) return;
  const validStr = [1, 1.5, 2, 2.5, 3, 4];
  const sequence = history.map((color, idx) => {
    const raw = getCandleStrength(idx) || 2;
    const str = validStr.includes(raw) ? raw : 2;
    return color + str;
  }).join(' ');

  try {
    localStorage.setItem('cmd-bt-seq', sequence);
    localStorage.setItem('cmd-bt-seq-src', 'index');
    localStorage.setItem('cmd-bt-seq-ts', Date.now().toString());
    // Keep legacy key for backward compat
    localStorage.setItem('cmd-backtest-sequence', sequence);
  } catch(e) {}

  if (bc) {
    try { bc.postMessage({ type: 'bt-seq-update', sequence, src: 'index', count: history.length }); } catch(e) {}
  }
}
...
// ── Listen for edits from monitor AND candles from backtesting ──
var _lastProcessedSeqLogic = '';

function _applySequenceToHistory(seq) {
  if (!seq || seq === _lastProcessedSeqLogic) return;
  _lastProcessedSeqLogic = seq;
  const tokens = seq.trim().toUpperCase().split(/\s+/).filter(Boolean);
  const parsed = [];
  tokens.forEach(t => {
    const m = t.match(/^([GR])(\d+\.?\d*)$/);
    if (m) parsed.push({ color: m[1], str: parseFloat(m[2]) });
  });
  if (parsed.length <= history.length) return; // nothing new
  // Suppress re-write while we're adding
  _suppressSyncWrite = true;
  for (let i = history.length; i < parsed.length; i++) {
    const nc = parsed[i];
    console.log('[sync→index] Agregando vela desde BT:', nc.color, nc.str);
    quickAddCandle(nc.color, nc.str);
  }
  _suppressSyncWrite = false;
}

function setupEditListener() {
  // Monitor edits via BroadcastChannel
  ...
  if (channel) {
    channel.onmessage = (e) => {
      ...
      // Receive sequence from backtesting via same channel
      if (e.data?.type === 'bt-seq-update' && e.data.src === 'backtesting') {
        _applySequenceToHistory(e.data.sequence);
      }
    };
  }

  // Storage events: monitor edits + backtesting sequence
  window.addEventListener('storage', (e) => {
    ...
    if (e.key === 'cmd-bt-seq') {
      const src = localStorage.getItem('cmd-bt-seq-src');
      if (src === 'backtesting') _applySequenceToHistory(e.newValue);
    }
  });
}
setupEditListener();
